...
class SkipList:
    ...
    def _locate_node(self, key):
        ...
        for i in reversed(range(self.level)):
            ...
            while True:
                ...
                next_node = node.forward[i]                 # 临时右移一次
                if next_node.key >= key:                    # 当 next_node.key > key 时, 不需要再做后续匹配, 因为没有意义.
                    break                                   # 当 next_node.key == key 时, 不需要再做后续匹配, 也没有意义.

                node = next_node                            # 确定右移

            update_vector.append(node)                      # node.forward[i].key>=key 的 node 添加到 update_vector 中.

        update_vector.reverse()                             # 反转列表
    ...
    def delete(self, key):
        ...
        for i, update_node in enumerate(update_vector):
            ...
            # 已是该层级最右节点, 不满足继续比较的条件.
            if i >= update_node.level:
                continue

            # 不是该层级最右节点, 比较两个key是否相等,
            # 当 两个 key  不相等时, 表示不满足执行删除动作的条件.
            next_node = update_node.forward[i]
            if next_node.key != key:
                continue

            # 在第i层级链表中, 找到了 两个key 相等的节点.
            assert next_node == node
            assert id(next_node) == id(node)

            # 执行删除动作.
            if i >= next_node.level:                            # 已是最右节点
                update_node.forward = update_node.forward[:i]
            else:                                               # 不是最右节点
                update_node.forward[i] = next_node.forward[i]
    ...
    def insert(self, key, value):
        ...
        for i, update_node in enumerate(update_vector[:level]):
            ...
            if i >= update_node.level:                      # 已是最右节点
                ...
                next_node = update_node.forward[i]
                new_node.forward.append(next_node)
