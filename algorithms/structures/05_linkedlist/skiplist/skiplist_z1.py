...
class SkipList:
    ...
    def _locate_node(self, key):
        ...
        # 前置比较
        index_invalid = len(node.forward) <= 0
        if index_invalid:
    ...
    def delete(self, key):
        ...
        for i, update_node in enumerate(update_vector):
            ...
            key_equal = update_node.forward[i].key == key
            if not key_equal: continue

            if node.level > i:                                      # 不是最右节点
                update_node.forward[i] = node.forward[i]            # update_node.forward[i] == node
            else:
                update_node.forward = update_node.forward[:i]       # 最右节点
