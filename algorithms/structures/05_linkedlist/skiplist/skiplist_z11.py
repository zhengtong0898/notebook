...
class SkipList:
    ...
    def __str__(self):
        return f"<Node level='{self.level}' at {hex(id(self))}>"
    ...
    def _locate_node(self, key):
        ...
        for i in reversed(range(self.level)):
            while True:
                if i >= node.level: break
                next_node = node.forward[i]
    ...
    def delete(self, key):
        ...
        for i, update_node in enumerate(update_vector):
            if i >= update_node.level: continue
            next_node = update_node.forward[i]
            if next_node.key != key: continue
            assert node.key == next_node.key
